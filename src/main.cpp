...
Dataset createDataset() {
    std::vector<Dataset::DataPair> values(100);

    for (auto i : msl::range(values.size())) {
        auto &pair = values.at(i);
        pair.x = {0.1 * i};
        pair.y = {pair.x.front() * .9};
    }
...
int main(int, char **) {
    ...
    auto trainer = Trainer{node, optimizer, dataset, dataset.data.size() / 10};
